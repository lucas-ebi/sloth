    ...
    def __init__(self, xsd_schema: Union[str, Path]):
        ...
        try:
            ...
            # Handle string paths
                ...
                self.schema_doc = self._etree.fromstring(
                    self.xsd_schema.encode("utf-8")
                )
            else:
...
class YAMLSchemaValidator(SchemaValidator):
    ...
    def __init__(self, schema: Dict[str, Any]):
        ...
        try:
            ...
            raise ImportError(
                "PyYAML library is required. Install it using 'pip install pyyaml'."
            )
        ...
        try:
            ...
            raise ImportError(
                "jsonschema library is required. Install it using 'pip install jsonschema'."
            )
...
class CSVSchemaValidator(SchemaValidator):
    ...
    def validate(self, data: Any) -> ValidationResult:
        ...
        try:
            ...
            raise ImportError(
                "pandas library is required. Install it using 'pip install pandas'."
            )
                ...
                if col_name in self.expected_columns:
                    ...
                    if col_type:
                        try:
                            ...
                            errors.append(
                                f"Column '{col_name}' has invalid type: {str(e)}"
                            )
...
default_mmcif_json_nested_schema = _load_schema_file("mmcif_json_nested_schema.json")
default_mmcif_json_flat_schema = _load_schema_file("mmcif_json_flat_schema.json")


# Schema validator factory class
class SchemaValidatorFactory:
    """Factory class for creating schema validators based on format and structure."""

    @staticmethod
    def create_validator(
        format_type: DataSourceFormat, 
        structure: StructureFormat = StructureFormat.NESTED,
        custom_schema: Optional[Any] = None
    ) -> SchemaValidator:
        """
        Create a schema validator for the specified format and structure.

        Args:
            format_type: The data format type
            structure: The structure format (nested or flat)
        ...
        if format_type == DataSourceFormat.JSON:
            if custom_schema:
                ...
                schema = (default_mmcif_json_nested_schema 
                         if structure == StructureFormat.NESTED 
                         else default_mmcif_json_flat_schema)
            return JSONSchemaValidator(schema)

        elif format_type == DataSourceFormat.XML:
            if not custom_schema:
                # Load the appropriate XSD schema based on structure
                schema_filename = (
                    "mmcif_xml_nested_schema.xsd" 
                    if structure == StructureFormat.NESTED 
                    else "mmcif_xml_flat_schema.xsd"
                )
                xsd_schema_content = _load_text_file(schema_filename)
                if not xsd_schema_content:
                    schema_path = os.path.join(_get_schema_dir(), schema_filename)
                    raise ValueError(
                        f"Required XML schema file not found: {schema_path}"
                    )

                try:
                    # Pass the XSD content to the validator
